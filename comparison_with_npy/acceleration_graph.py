...
def calculate_acceleration(timestep, pointset):
    velocity_1 = np.array(pointset[(timestep - 2) * offset][PARTICLE_NUM]) - np.array(pointset[(timestep - 1) * offset][PARTICLE_NUM])
    velocity_2 = np.array(pointset[timestep * offset][PARTICLE_NUM]) - np.array(pointset[(timestep - 1) * offset][PARTICLE_NUM])
...
for test_case_num, test_case in tqdm(enumerate(DNRI_TEST_CASES[:TEST_ANIMATIONS])):
    ...
    plt.plot(predicted_timesteps, ours_x_predicted_accelerations, label='Ours')
    plt.plot(nri_predicted_timesteps, dnri_x_predicted_accelerations, label='dNRI')
    plt.plot(nri_predicted_timesteps, nri_x_predicted_accelerations, label='NRI')
    plt.xlabel('Timestep')
    plt.ylabel('X Acceleration')
    plt.legend()
    plt.savefig(f'x_acceleration_{test_case_num}.png', dpi=600)

    plt.clf()
    plt.plot(timesteps, y_accelerations, label='Ground Truth')
    plt.plot(predicted_timesteps, ours_y_predicted_accelerations, label='Ours')
    plt.plot(nri_predicted_timesteps, dnri_y_predicted_accelerations, label='dNRI')
    plt.plot(nri_predicted_timesteps, nri_y_predicted_accelerations, label='NRI')
    plt.xlabel('Timestep')
    plt.ylabel('Y Acceleration')
    plt.legend()
    plt.savefig(f'y_acceleration_{test_case_num}.png', dpi=600)
